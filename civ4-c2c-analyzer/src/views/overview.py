...
def display_overview(df_player_stats, df_city_history, players_data, cities_data, data_directory):
    ...
    with st.expander("Show/Hide Graph Options", expanded=False):
        show_ov_summary = st.checkbox("Show summary", value=GRAPH_CONFIG["overview"]["show_summary"])
        show_ov_trends = st.checkbox("Show trends", value=GRAPH_CONFIG["overview"]["show_trends"])
        show_ov_map = st.checkbox("Show map", value=GRAPH_CONFIG["overview"]["show_map"])

    # General metrics (shown if show_summary is True)
    if show_ov_summary:
    ...
    # Global progression plots (shown if show_trends is True)
    if show_ov_trends:
    ...
    # Map display (shown if show_map is True)
    if show_ov_map:
        # Add your map display code here
        st.info("Map display not implemented yet.")